# 控制变量: 年龄 (age) + BMI (bmi) + 性别 (gender) + 城镇化/贫困指数 (townsend)
...
def analyze_smoking_by_groups(file_path):
    ...
    for col in ['age', 'bmi', 'townsend', 'gender']:
    ...
    # 5. 运行逻辑回归
    formula = "label ~ C(smoking_group, Treatment(reference='None (不吸)')) + age + bmi"
    if 'gender' in df.columns: formula += " + C(gender)"
    if 'townsend' in df.columns: formula += " + townsend"
    ...
    try:
        ...
        for i, label in enumerate(labels[1:]):
            try:
                ...
                print(f"⚠️ 警告: 组别 {label} 样本太少，无法计算")
...
def plot_or_results(df):
    ...
    colors = []
    for p, g in zip(df['P-val'], df['Group']):
        if g == 'None (不吸)':
    ...
    plt.title('不同程度吸烟占比与患癌风险对比', fontsize=14)
    ...
    for bar, or_val, p_val, upper_val, group in zip(bars, df['OR'], df['P-val'], df['Upper'], df['Group']):
        ...
        text_str = f"{or_val:.2f}{star}"
        text_y = max(or_val, upper_val) + (max_height * 0.02)
        plt.text(bar.get_x() + bar.get_width() / 2., text_y, text_str, ha='center', va='bottom', fontweight='bold',
                 fontsize=11)

    plt.figtext(0.99, 0.01, '* P<0.05, ** P<0.01, *** P<0.001', horizontalalignment='right', fontsize=9, color='gray')
    plt.tight_layout()
    plt.show()
...
def plot_trend_curve(df):
    ...
    for x, y, p_val, group in zip(x_indices, df['OR'], df['P-val'], df['Group']):
        star = get_significance_star(p_val) if group != 'None (不吸)' else ""
        plt.text(x, y + 0.08, f"{y:.2f}{star}", ha='center', va='bottom', fontweight='bold', color='#d62728',
                 fontsize=11)
...
if __name__ == "__main__":
    fpath = "/Users/tongan/tongan/project/ukbank_rectal cancer_analysis/dataset/merged_with_C18_flag_label_final.csv"
